...
#define to_spinal_udc_req(req)                                                 \
	container_of((req), struct spinal_udc_req, usb_req)

struct spinal_udc_descriptor {
	struct list_head
		udc_node; //Used for both allocation queue and ep->descriptors
...
static int __spinal_udc_ep0_queue(struct spinal_udc_ep *ep0,
...
static void spinal_udc_clear_stall_all_ep(struct spinal_udc *udc)
{
	...
	for (i = 0; i < SPINAL_UDC_MAX_ENDPOINTS; i++) {
		ep = &udc->ep[i];
		spinal_udc_ep_status_mask(
			ep, ~(USB_DEVICE_EP_STALL | USB_DEVICE_EP_PHASE(1)), 0);
	}
}
...
static void spinal_udc_ep0_stall(struct spinal_udc *udc, bool throw_desc)
{
	...
	spinal_udc_ep_status_mask(udc->ep, ~(throw_desc ? 0xFFF0 : 0),
				  USB_DEVICE_EP_STALL);
	if (readl(udc->addr + USB_DEVICE_INTERRUPT) & USB_DEVICE_IRQ_SETUP) {
...
static void spinal_udc_ep_stall(struct spinal_udc *udc,
				struct spinal_udc_ep *ep, bool throw_desc)
{
	if (ep->epnumber == 0) {
	...
	spinal_udc_ep_status_mask(ep, ~(throw_desc ? 0xFFF0 : 0),
				  USB_DEVICE_EP_STALL);
}

static void spinal_udc_ep_unstall(struct spinal_udc *udc,
				  struct spinal_udc_ep *ep, bool clear_phase)
{
	dev_dbg(udc->dev, "%s %d %d\n", __func__, ep->epnumber, clear_phase);
	spinal_udc_ep_status_mask(ep,
				  ~(USB_DEVICE_EP_STALL |
				    (clear_phase ? USB_DEVICE_EP_PHASE(1) : 0)),
				  0);
}

static void spinal_udc_set_address_completion(struct usb_ep *_ep,
...
static void spinal_udc_ep_desc_free(struct spinal_udc_ep *ep,
...
static void spinal_udc_done(struct spinal_udc_ep *ep,
			    struct spinal_udc_req *req, int status)
{
	struct spinal_udc *udc = ep->udc;
	dev_dbg(udc->dev, "%s %d %d\n", __func__, ep->epnumber,
		req->usb_req.actual);
	...
	if (!list_empty(&req->descriptors)) {
		...
		while (!list_empty(&req->descriptors)) {
			...
			u32 next, tmp;
			//
			desc = list_first_entry(&req->descriptors,
						struct spinal_udc_descriptor,
						req_node);
			entry = list_first_entry(&ep->descriptors,
						 struct spinal_udc_descriptor,
						 udc_node);
			next = readl(desc->mapping + 4) & 0xFFF0;
			if (entry == desc) { //On the hardware head
				tmp = readl(udc->addr + ep->epnumber * 4) &
				      ~0xFFF0;
				writel(tmp | next,
				       udc->addr + ep->epnumber * 4);
	...
	dev_dbg(udc->dev, "%s reqs done left %d %d\n", __func__, ep->epnumber,
		ep->pending_reqs_done);

	if (req->usb_req.complete) {
		spin_unlock(&udc->lock);
		dev_dbg(udc->dev, "%s complete call %x L=%d\n", __func__,
			(u32)req->usb_req.complete, req->usb_req.actual);
...
static void spinal_udc_nuke(struct spinal_udc_ep *ep, int status)
{
	...
	if (tmp) {
		dev_dbg(ep->udc->dev, "%s DESC status : %x %x %x\n", __func__,
			readl(udc->addr + tmp + 0), readl(udc->addr + tmp + 4),
			readl(udc->addr + tmp + 8));
	}
	...
	while (!list_empty(&ep->reqs)) {
		req = list_first_entry(&ep->reqs, struct spinal_udc_req,
				       ep_node);
...
static void spinal_udc_get_status(struct spinal_udc *udc)
{
	...
	case USB_RECIP_ENDPOINT:
		...
		halt = readl(udc->addr + target_ep->epnumber * 4) &
		       USB_DEVICE_EP_STALL;
...
stall:
	dev_err(udc->dev, "Can't respond to getstatus request %d %d %d %d\n",
		udc->setup.bRequestType & USB_RECIP_MASK,
		udc->setup.wIndex & USB_DIR_IN, target_ep->is_in, ret);
	spinal_udc_ep0_stall(udc, 1);
}
		...
		if (!udc->setup.wValue) {
			...
			if (!endpoint) {
				...
				if (flag) {
					...
					spinal_udc_ep_unstall(udc, target_ep,
							      1);
				}
			}
		}
...
static void spinal_udc_ep0_status_completion(struct usb_ep *_ep,
					     struct usb_request *_req)
{
	struct spinal_udc_ep *ep = to_spinal_udc_ep(_ep);
	struct spinal_udc *udc = ep->udc;
	...
	if (usb_data_req->complete &&
	    usb_data_req->complete != usb_data_req->complete) {
...
static void spinal_udc_ep0_data_completion(struct usb_ep *_ep,
...
static void spinal_udc_setup_irq(struct spinal_udc *udc)
{
	...
	dev_dbg(udc->dev, "%s USB setup %08x %08x\n", __func__,
		cpu_to_be32(payload[0]), cpu_to_be32(payload[1]));
	...
	case USB_REQ_GET_STATUS:
		...
		if ((udc->setup.bRequestType & (USB_DIR_IN | USB_TYPE_MASK)) !=
...
static void spinal_udc_ep_irq(struct spinal_udc_ep *ep)
{
	...
	while (1) {
		struct spinal_udc_req *req;
		struct spinal_udc_descriptor *desc;
		req = list_first_entry_or_null(&ep->reqs, struct spinal_udc_req,
					       ep_node);
		if (!req)
			return;
		while (1) {
			...
			desc = list_first_entry_or_null(
				&req->descriptors, struct spinal_udc_descriptor,
				req_node);
			if (!desc) {
			...
			if (((status >> 16) & 0xF) == USB_DEVICE_CODE_NONE)
				return;

			length = (status & 0xFFFF) - desc->offset;
			//            dev_dbg(udc->dev, "%s %x %d\n", __func__, status, length);
			if (!ep->is_in) {
				memcpy(req->usb_req.buf + req->usb_req.actual,
				       desc->mapping + 12 + desc->offset,
				       length);
			}
...
static void spinal_udc_epconfig(struct spinal_udc_ep *ep,
				struct spinal_udc *udc)
{
...
static int spinal_udc_pullup(struct usb_gadget *gadget, int is_on)
{
	...
	spin_lock_irqsave(&udc->lock, flags);
	writel(is_on ? USB_DEVICE_PULLUP_ENABLE : USB_DEVICE_PULLUP_DISABLE,
	       udc->addr + USB_DEVICE_CONFIG);
	spin_unlock_irqrestore(&udc->lock, flags);
	return 0;
}
				  ...
				  const struct usb_endpoint_descriptor *desc)
{
	...
	writel(USB_DEVICE_EP_ENABLE | USB_DEVICE_EP_PHASE(0) |
		       USB_DEVICE_EP_MAX_PACKET_SIZE(maxpacket),
	       udc->addr + ep->epnumber * 4);
...
static void spinal_udc_descriptor_push(struct spinal_udc *udc,
				       struct spinal_udc_ep *ep,
				       struct spinal_udc_descriptor *desc)
{
	//    dev_dbg(udc->dev, "%s %d\n", __func__, ep->epnumber);
	if (!list_empty(&ep->descriptors)) {
		struct spinal_udc_descriptor *last =
			list_last_entry(&ep->descriptors,
					struct spinal_udc_descriptor, udc_node);
		//dev_dbg(udc->dev, "%s push tail\n", __func__);
		writel(desc->address |
			       ((last->length_deployed + last->offset) << 16),
		       last->mapping + 4);
	} else {
...
static void spinal_udc_ep_link_head(struct spinal_udc_ep *ep)
{
	...
	if (list_empty(&ep->descriptors))
		return;
	head = list_first_entry(&ep->descriptors, struct spinal_udc_descriptor,
				udc_node);
...
static void spinal_udc_req_init(struct spinal_udc_req *req,
				struct spinal_udc_ep *ep)
{
 ...
 */
static void spinal_udc_ep_free_request(struct usb_ep *_ep,
				       struct usb_request *_req)
{
	struct spinal_udc_ep *ep = to_spinal_udc_ep(_ep);
	struct spinal_udc *udc = ep->udc;
...
static void spinal_udc_ep_desc_refill(struct spinal_udc_ep *ep)
{
	...
	while (ep->descriptor_count != EP_DESC_MAX) {
		...
		dev_dbg(udc->dev, "dp_large %s %d", __func__,
			!list_empty(&udc->dp_large));

		if (left >= DESC_LARGE_SIZE - 4 &&
		    !list_empty(&udc->dp_large)) {
			dev_dbg(udc->dev, "%s dp_large picked", __func__);
			desc = list_first_entry(&udc->dp_large,
						struct spinal_udc_descriptor,
						udc_node);
			//            printk("L %d %d\n", ep->epnumber, req->usb_req.length);
			   ...
			   (ep->epnumber == 0 ||
			    udc->dp_small.next !=
				    udc->dp_small
					    .prev)) { //Only allow EP0 to take the last descriptor.
			desc = list_first_entry(&udc->dp_small,
						struct spinal_udc_descriptor,
						udc_node);
		} else {
			dev_dbg(udc->dev, "%s EMPTY !!!!", __func__);
			printk("spinal_udc running out of descriptor !!\n");
			if (ep->descriptor_count ==
			    0) { //Will need some wakeup for a refill later on.
		...
		packet_end =
			length == left && ep->is_in &&
			!(ep->is_in && !req->usb_req.zero) &&
			!(ep->epnumber == 0 &&
			  req->commited_length + length >= udc->setup.wLength);
		writel((USB_DEVICE_CODE_NONE << 16) | (offset),
		       desc->mapping + 0);
		writel(((length + offset) << 16), desc->mapping + 4);
		writel((ep->is_in ? USB_DEVICE_DESC_IN : USB_DEVICE_DESC_OUT) |
			       (packet_end ? 0 :
						   USB_DEVICE_DESC_COMPL_ON_FULL) |
			       (desc->req_completion && ep->epnumber == 0 ?
					      USB_DEVICE_DESC_DATA1_COMPLETION :
					      0) |
			       USB_DEVICE_DESC_INTERRUPT,
		       desc->mapping + 8);
		...
		dev_dbg(udc->dev,
			"%s commited %d %d %d %d %d left %d z=%d s=%d\n",
			__func__, ep->epnumber, ep->is_in, length,
			desc->req_completion, packet_end,
			req->usb_req.length - req->commited_length,
			req->usb_req.zero, req->usb_req.short_not_ok);
	}
}
				  ...
				  struct spinal_udc_req *req)
{
	...
	dev_dbg(udc->dev, "%s %d pending %d", __func__, req->usb_req.length,
		ep0->pending_reqs_done);
	...
	if (udc->ep0_state == EP0_STATE_DATA) {
		...
		if (req->usb_req.length == 0) {
			...
			req->usb_req.status = 0;
			ep0->pending_reqs_done -= 1;
			spinal_udc_ep0_data_completion(&ep0->ep_usb,
						       &req->usb_req);
		} else {
...
static const struct usb_ep_ops spinal_udc_ep0_ops = {
	//TOOD
...
static int spinal_udc_eps_init(struct spinal_udc *udc)
{
	...
	udc->ep_count = 16;
	udc->ep = devm_kzalloc(udc->dev,
			       sizeof(struct spinal_udc_ep) * udc->ep_count,
			       GFP_KERNEL);
	if (!udc->ep)
		return -ENOMEM;
	...
	for (ep_number = 0; ep_number < udc->ep_count; ep_number++) {
		...
		if (ep_number) {
			...
			usb_ep_set_maxpacket_limit(
				&ep->ep_usb,
				EP_MAX_PACKET); // (unsigned short) ~0
...
static int spinal_udc_probe(struct platform_device *pdev)
{
	...
	writel(USB_DEVICE_INTERRUPT_DISABLE || USB_DEVICE_PULLUP_DISABLE,
	       udc->addr + USB_DEVICE_CONFIG);
...
static const struct of_device_id usb_of_match[] = {
	...
	{
		.compatible = "spinalhdl,usb_udc-1.0",
	},
	{ /* end of list */ },
};
