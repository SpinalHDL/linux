...
#define DESC_HEADER_SIZE 0xC
#define DESC_SMALL_SIZE 0x64
...
static void spinal_udc_done(struct spinal_udc_ep *ep, struct spinal_udc_req *req, int status)
{
    ...
    if (req->usb_req.complete) {
        ...
        dev_dbg(udc->dev, "%s complete call %x\n", __func__, (u32)req->usb_req.complete);
...
static void spinal_udc_ep_refresh(struct spinal_udc_ep *ep){
    struct spinal_udc *udc = ep->udc;
...
//    u32 length;
//    u8 *corebuf;
    dev_dbg(udc->dev, "%s %d", __func__, req->usb_req.length);
...
//    int  ret;
//    unsigned long flags;
    dev_dbg(udc->dev, "%s UNIMPLMENTED", __func__);
//
//    if (!ep->desc) {
