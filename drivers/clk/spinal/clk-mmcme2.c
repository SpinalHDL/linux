...
void spinal_mmcme2_solve(struct clk_hw *hw, unsigned long rate, unsigned long *parent_rate, struct spinal_mmcme2_state* result){
    u32 mul;
    u32 div;
    u32 div0;
    u64 best_score = -1;
...
static struct clk_hw *of_clk_spinal_get(struct of_phandle_args *clkspec, void *data)
{
...
int spinal_mmcme2_set_rate(struct clk_hw *hw, unsigned long rate, unsigned long parent_rate){
    struct spinal_mmcme2 *spinal_mmcme2 = container_of(hw, struct spinal_mmcme2, clkout0);
    struct spinal_mmcme2_state state;
    u32 tmp;
    ...
    dbus_write(HIGH_TIME(tmp/2) | LOW_TIME(tmp/2 + (tmp%2)) | (clkfbout_r1 & 0xF000), CLKFBOUT_R1);

    tmp = state.div0/FRAC_FACTOR;
    dbus_write(HIGH_TIME(tmp/2) | LOW_TIME(tmp/2 + (tmp%2)) | (clkout0_r1 & 0xF000), CLKOUT0_R1);

    dbus_write(HIGH_TIME(state.div/2) | LOW_TIME(state.div/2 + (state.div%2)) | (divin & 0xE000), DIVIN);
...
//    .prepare = spinal_mmcme2_prepare,
//    .unprepare = spinal_mmcme2_unprepare,
//    .is_prepared = spinal_mmcme2_is_prepared,
...
{
//    struct spinal *spinal = dev_get_drvdata(pdev->dev.parent);
//    struct i2c_client *client = spinal->i2c;
//    struct device_node *node = client->dev.of_node;
    ...
    spinal_mmcme2 = devm_kzalloc(&pdev->dev,
                    sizeof(*spinal_mmcme2), GFP_KERNEL);
    if (!spinal_mmcme2)
        return -ENOMEM;
...
//    of_property_read_string(pdev->dev.of_node, "clock-output-names", &init.name);
...
//module_platform_driver(spinal_mmcme2_driver);
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:spinal-mmcme2");
