...
#define CONFIG_SERIAL_SPINAL_LIB_UART_MAXPORTS 32
...
{
	return (spinal_lib_uart_tx_availability(port) &
		SPINAL_LIB_UART_STATUS_TX_BUSY) ?
			     0 :
			     TIOCSER_TEMT;
}
...
static void spinal_lib_uart_break_ctl(struct uart_port *port, int break_state)
{
	...
	writel(break_state != 0 ? SPINAL_LIB_UART_MISC_BREAK_ENABLE :
					SPINAL_LIB_UART_MISC_BREAK_DISABLE,
					...
					struct ktermios *old)
{
	...
	config |= (termios->c_cflag & CSTOPB) ?
				SPINAL_LIB_UART_FRAME_CONFIG_STOP2 :
				SPINAL_LIB_UART_FRAME_CONFIG_STOP1;
	if (termios->c_cflag & CSTOPB) {
		config |= (termios->c_cflag & PARODD) ?
					SPINAL_LIB_UART_FRAME_CONFIG_ODD :
					SPINAL_LIB_UART_FRAME_CONFIG_EVEN;
	}

	spin_lock_irqsave(&port->lock, flags);
	uart_update_timeout(port, termios->c_cflag, baud);
	writel(baudclk - 1, port->membase + SPINAL_LIB_UART_CLOCK_DIVIDER);
...
static void spinal_lib_uart_rx_chars(struct spinal_lib_uart *pp)
{
	...
	//	printk("RX => %x %x", flags, pp->break_on_rx);
	if (!pp->break_on_rx) {
	...
	if (!pp->break_on_rx)
		while ((count = spinal_lib_uart_rx_occupancy(port))) {
			while (count--) {
				ch = readl(port->membase +
					   SPINAL_LIB_UART_DATA);
				flag = TTY_NORMAL;
				port->icount.rx++;
...
static irqreturn_t spinal_lib_uart_interrupt(int irq, void *data)
{
	...
	struct spinal_lib_uart *pp =
		container_of(port, struct spinal_lib_uart, port);
	unsigned int isr = 0;

	//	isr = spinal_lib_uart_readl(port, SPINAL_LIB_UART_STATUS_REG) & pp->imr;
...
{
	port->type = PORT_SPINALHDL_UART;

	//TODO
	/* Clear mask, so no surprise interrupts. */
	//	spinal_lib_uart_writel(port, 0, SPINAL_LIB_UART_CONTROL_REG);
	/* Clear status register */
	//	spinal_lib_uart_writel(port, 0, SPINAL_LIB_UART_STATUS_REG);
}
...
static int spinal_lib_uart_startup(struct uart_port *port)
{
	...
	//		int ret;
	//
	//		ret = request_irq(port->irq, spinal_lib_uart_interrupt, 0,
	//				DRV_NAME, port);
	//		if (ret) {
...
{
	return (port->type == PORT_SPINALHDL_UART) ? "SpinalHDL UART" : NULL;
}
...
{
	if ((ser->type != PORT_UNKNOWN) && (ser->type != PORT_SPINALHDL_UART))
		return -EINVAL;
	return 0;
}
...
static void spinal_lib_uart_poll_put_char(struct uart_port *port,
					  unsigned char c)
{
	while (!(spinal_lib_uart_readl(port, SPINAL_LIB_UART_STATUS_REG) &
...
#if defined(CONFIG_SERIAL_SPINAL_LIB_UART_CONSOLE)
...
static void spinal_lib_uart_console_write(struct console *co, const char *s,
					  unsigned int count)
{
	struct uart_port *port = &(spinal_lib_uart_ports + co->index)->port;

	uart_console_write(port, s, count, spinal_lib_uart_console_putc);
}

static int __init spinal_lib_uart_console_setup(struct console *co,
						char *options)
{
	struct uart_port *port;
	...
	if (co->index < 0 ||
	    co->index >= CONFIG_SERIAL_SPINAL_LIB_UART_MAXPORTS)
		return -EINVAL;
						 ...
						 const char *options)
{
	...
	if (dev->baud) {
		...
		spinal_lib_uart_writel(port, baudclk,
				       SPINAL_LIB_UART_DIVISOR_REG);
	}
...
OF_EARLYCON_DECLARE(uart, "spinal-lib,uart-1.0",
		    spinal_lib_uart_earlycon_setup);
...
static struct uart_driver spinal_lib_uart_driver = {
	...
	.nr = CONFIG_SERIAL_SPINAL_LIB_UART_MAXPORTS,
...
static int spinal_lib_uart_probe(struct platform_device *pdev)
{
	...
	//	if (i < 0 || i >= CONFIG_SERIAL_SPINAL_LIB_UART_MAXPORTS)
	//		return -EINVAL;
	...
	uart_add_one_port(&spinal_lib_uart_driver, port);
	return 0;
