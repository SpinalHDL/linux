...
static void dmasg_input_memory(void __iomem *base, u32 channel, u32 address,
			       u32 byte_per_burst)
{
	write_u32(address, dmasg_ca + DMASG_CHANNEL_INPUT_ADDRESS);
	write_u32(DMASG_CHANNEL_INPUT_CONFIG_MEMORY |
			  ((byte_per_burst - 1) & 0xFFF),
		  dmasg_ca + DMASG_CHANNEL_INPUT_CONFIG);
}

// byte_per_burst need to be a power of two, can be set to zero if the channel has hardcoded burst length
static void dmasg_output_memory(void __iomem *base, u32 channel, u32 address,
				u32 byte_per_burst)
{
	write_u32(address, dmasg_ca + DMASG_CHANNEL_OUTPUT_ADDRESS);
	write_u32(DMASG_CHANNEL_OUTPUT_CONFIG_MEMORY |
			  ((byte_per_burst - 1) & 0xFFF),
		  dmasg_ca + DMASG_CHANNEL_OUTPUT_CONFIG);
}
...
static void dmasg_input_stream(void __iomem *base, u32 channel, u32 port,
			       u32 wait_on_packet, u32 completion_on_packet)
{
	write_u32(port << 0, dmasg_ca + DMASG_CHANNEL_INPUT_STREAM);
	write_u32(
		DMASG_CHANNEL_INPUT_CONFIG_STREAM |
			(completion_on_packet ?
				       DMASG_CHANNEL_INPUT_CONFIG_COMPLETION_ON_PACKET :
				       0) |
			(wait_on_packet ?
				       DMASG_CHANNEL_INPUT_CONFIG_WAIT_ON_PACKET :
				       0),
		dmasg_ca + DMASG_CHANNEL_INPUT_CONFIG);
}
...
static void dmasg_output_stream(void __iomem *base, u32 channel, u32 port,
				u32 source, u32 sink, u32 last)
{
	write_u32(port << 0 | source << 8 | sink << 16,
		  dmasg_ca + DMASG_CHANNEL_OUTPUT_STREAM);
	write_u32(DMASG_CHANNEL_OUTPUT_CONFIG_STREAM |
			  (last ? DMASG_CHANNEL_OUTPUT_CONFIG_LAST : 0),
		  dmasg_ca + DMASG_CHANNEL_OUTPUT_CONFIG);
}
...
static void dmasg_direct_start(void __iomem *base, u32 channel, u32 bytes,
			       u32 self_restart)
{
	write_u32(bytes - 1, dmasg_ca + DMASG_CHANNEL_DIRECT_BYTES);
	write_u32(DMASG_CHANNEL_STATUS_DIRECT_START |
			  (self_restart ? DMASG_CHANNEL_STATUS_SELF_RESTART :
						0),
		  dmasg_ca + DMASG_CHANNEL_STATUS);
}
...
static void dmasg_linked_list_start(void __iomem *base, u32 channel, u32 head)
{
	...
	write_u32(DMASG_CHANNEL_STATUS_LINKED_LIST_START,
		  dmasg_ca + DMASG_CHANNEL_STATUS);
}
...
static void dmasg_interrupt_pending_clear(void __iomem *base, u32 channel,
					  u32 mask)
{
...
{
	return read_u32(dmasg_ca + DMASG_CHANNEL_STATUS) &
	       DMASG_CHANNEL_STATUS_BUSY;
}
...
static void dmasg_buffer(void __iomem *base, u32 channel, u32 fifo_base,
			 u32 fifo_bytes)
{
	write_u32(fifo_base << 0 | (fifo_bytes - 1) << 16,
		  dmasg_ca + DMASG_CHANNEL_FIFO);
}
...
static struct spinal_lib_dmasg_segment *
spinal_lib_dmasg_alloc_segment(struct spinal_lib_dmasg_chan *chan)
{
...
static int
spinal_lib_dmasg_free_tx_descriptor(struct spinal_lib_dmasg_chan *chan,
				    struct spinal_lib_dmasg_tx_descriptor *desc)
{
	struct spinal_lib_dmasg_segment *segment;
...
static void
spinal_lib_dmasg_cyclic_segment_update(struct spinal_lib_dmasg_chan *chan,
				       bool init)
{
	...
	while (1) {
		...
		// Update segment
		bytes = min_t(u32,
			      min_t(u32, SPINAL_LIB_DMASG_MAX_TRANS_LEN,
				    desc->period_left),
			      desc->buffer_left);
		segment->hw.control = bytes - 1;
		segment->hw.from =
			desc->buf_addr + (desc->buffer_len - desc->buffer_left);
		segment->hw.status = 0;
...
{
	struct spinal_lib_dmasg_chan *chan =
		(struct spinal_lib_dmasg_chan *)data;
...
static irqreturn_t spinal_lib_dmasg_interrupt(int irq, void *dev_id)
{
	...
	dmasg_interrupt_config(
		chan->priv->regs, chan->hardware_id,
		DMASG_CHANNEL_INTERRUPT_DESCRIPTOR_COMPLETION_MASK);
				       ...
				       int software_id)
{
	...
	chan->segment_pool =
		dma_pool_create("spinal_lib_dmasg_segment_pool", chan->dev,
	...
	tasklet_init(&chan->tasklet, spinal_lib_dmasg_do_tasklet,
		     (unsigned long)chan);
...
static int spinal_lib_dmasg_terminate_all(struct dma_chan *dchan)
{
	...
	if (chan->current_descriptor) {
		spinal_lib_dmasg_free_tx_descriptor(chan,
						    chan->current_descriptor);
...
static void spinal_lib_dmasg_issue_pending(struct dma_chan *dchan)
{
	...
	desc = list_first_entry(&chan->pending_list,
				struct spinal_lib_dmasg_tx_descriptor, node);
	head_segment = list_first_entry(&desc->segments,
					struct spinal_lib_dmasg_segment, node);
	...
	dmasg_linked_list_start(chan->priv->regs, chan->hardware_id,
				(u32)head_segment->phys);
...
of_dma_spinal_lib_xlate(struct of_phandle_args *dma_spec, struct of_dma *ofdma)
{
	...
	return dma_get_slave_channel(
		&priv->chan[chan_id]
			 .common); // dma_get_slave_channel(&priv->chan[chan_id]->common);
}

static dma_cookie_t
spinal_lib_dmasg_tx_submit(struct dma_async_tx_descriptor *tx)
{
...
spinal_lib_dmasg_prep_dma_cyclic(struct dma_chan *dchan, dma_addr_t buf_addr,
...
static struct dma_async_tx_descriptor *
spinal_lib_dmasg_prep_slave_sg(struct dma_chan *dchan, struct scatterlist *sgl,
			       unsigned int sg_len,
			       enum dma_transfer_direction direction,
...
static int spinal_lib_dmasg_probe(struct platform_device *pdev)
{
	...
	priv->chan = devm_kzalloc(
		&pdev->dev, sizeof(*priv->chan) * priv->chan_count, GFP_KERNEL);
	if (!priv->chan)
		return -ENOMEM;
	...
	priv->common.device_alloc_chan_resources =
		spinal_lib_dmasg_alloc_chan_resources;
	priv->common.device_free_chan_resources =
		spinal_lib_dmasg_free_chan_resources;
...
MODULE_DESCRIPTION("SpinalHDL DMA-SG IP driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("spinalhdl-dmasg");
